...
logging.basicConfig(filename="imagescrapper.log",level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def download_file(driver, filename=None):
    """
    ...
    for image in images:
        ...
        if url is None:
            continue
        try:
            urlretrieve(url=url, filename=os.path.join(downloadsFolder,f"{'_'.join(searchq.split(' '))}_{images.index(image)}.jpg"))
            logging.info(f"Successfully downloaded file from '{url}' and saved as '{filename}'")
